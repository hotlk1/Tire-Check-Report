...
const STATES: TireAssetState[] = ["storage", "unassigned", "damaged", "removed", "disposed", "lost"];
...
export async function setTireStateAction(form: FormData) {
  ...
  await setTireState(session.scope, { tireId, state, storageLocation: str(form, "storageLocation"), note: str(form, "note") }, label(session));
  revalidatePath(to);
  redirect(`${to}?saved=1`);
}
...
export async function registerTireAction(form: FormData) {
  ...
  const created = await createUnmountedTire(session.scope, { make: str(form, "make"), model: str(form, "model"), size: str(form, "size"), tireVariantId: null, serial: str(form, "serial"), notes: str(form, "notes"), storageLocation: str(form, "storageLocation") }, label(session));
