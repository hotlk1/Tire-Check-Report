...
export async function listTireEntries(scope: Scope & { tenantId: string }, f: TireFilters = {}): Promise<{ rows: TireEntryRow[]; total: number }> {
  ...
  return withScope(scope, async (tx) => {
    const where = tx`
      te.tenant_id = ${scope.tenantId} and i.status = 'submitted' and te.absent = false
    ...
    const rows = await tx<TireEntryRow[]>`
      select te.id, te.inspection_id, i.submitted_at, a.unit_number, te.asset_id, te.tire_number, te.position_code, te.psi::float8 as psi, te.tread_32nds, te.damage, te.absent, te.overall_status,
             d.full_name as driver_name, (select count(*)::int from photos p where p.tire_entry_id = te.id) as photos
      from tire_entries te join inspections i on i.id = te.inspection_id left join assets a on a.id = te.asset_id left join drivers d on d.id = i.driver_id
